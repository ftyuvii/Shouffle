...
class SetupView(discord.ui.View):
    ...
    def __init__(self, cog: "Welcome", ctx: commands.Context, config: dict):
    ...
    async def btn_save(self, interaction: discord.Interaction, _: discord.ui.Button):
        ...
        for item in self.children:
            item.disabled = True
        tip = (
            "Run `?welcomechannel #channel` to activate welcoming."
...
class Welcome(commands.Cog):
    ...
    async def welcometest(self, ctx: commands.Context):
        """Send a test welcome message for the current member."""
        data = _load()
        cfg  = data.get(str(ctx.guild.id))
